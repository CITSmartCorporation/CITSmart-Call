...
- (IBAction)onRemoteProvisioningDownloadClick:(id)sender {
    ...
    @try {
        NSString *serverURL = [self findTextField:ViewElement_URL].text;
        NSString *serverUsername = [self findTextField:ViewElement_Username].text;
        NSString *serverPassword = [self findTextField:ViewElement_Password].text;
        
        NSURL *URLLogin                      = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", serverURL, @"/services/login"]];
        AFHTTPSessionManager *managerLogin   = [AFHTTPSessionManager manager];
        
        [managerLogin setRequestSerializer:[AFJSONRequestSerializer serializer]];
        [managerLogin setResponseSerializer:[AFJSONResponseSerializer serializer]];
        
        [managerLogin POST:URLLogin.absoluteString
                parameters:@{@"userName":serverUsername, @"password":serverPassword, @"platform":@"", @"mediaType":@"application/json"}
                  progress:nil
                   success:^(NSURLSessionTask *taskLogin, id responseObjectLogin) {
                       NSDictionary *jsonLogin = (NSDictionary *) responseObjectLogin;
                       NSString *sessionID = [NSString stringWithFormat:@"%@", [jsonLogin objectForKey:@"sessionID"]];
                       
                       NSURL *URLVoIP                      = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", serverURL, @"/services/voip/query"]];
                       AFHTTPSessionManager *managerVoIP   = [AFHTTPSessionManager manager];
                       
                       [managerVoIP setRequestSerializer:[AFJSONRequestSerializer serializer]];
                       [managerVoIP setResponseSerializer:[AFJSONResponseSerializer serializer]];
                       
                       [managerVoIP POST:URLVoIP.absoluteString
                              parameters:@{@"sessionID":sessionID}
                                progress:nil
                                 success:^(NSURLSessionTask *taskVoIP, id responseObjectVoIP) {
                                     NSDictionary *jsonVoIP = (NSDictionary *) responseObjectVoIP;
                                     
                                     NSDictionary *result = (NSDictionary *) [jsonVoIP objectForKey:@"result"];
                                     
                                     NSString *enable = [NSString stringWithFormat:@"%@", [result objectForKey:@"enable"]];
                                     NSString *username = [NSString stringWithFormat:@"%@", [result objectForKey:@"username"]];
                                     NSString *pwd = [NSString stringWithFormat:@"%@", [result objectForKey:@"password"]];
                                     
                                     NSString *domain = @"";
                                     NSString *port = [NSString stringWithFormat:@"%@", [result objectForKey:@"port"]];
                                     
                                     if (port != nil && ![port isEqualToString:@""]) {
                                         domain = [NSString stringWithFormat:@"%@:%@", [result objectForKey:@"domain"], port];
                                     } else {
                                 ...
                                 failure:^(NSURLSessionTask *operationVoIP, NSError *errorVoIP) {
                                     NSLog(@"Error: %@", errorVoIP);
                                     [self displayAssistantConfigurationError];
                                 }
                        ];
                   }
                   failure:^(NSURLSessionTask *operationLogin, NSError *errorLogin) {
                       NSLog(@"Error: %@", errorLogin);
                       [self displayAssistantConfigurationError];
                   }
         ];
    }
